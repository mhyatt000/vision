...
from general.structures.bounding_box import BoxList, _onnx_clip_boxes_to_image
from general.structures.boxlist_ops import (
    boxlist_ml_nms,
    boxlist_nms,
    cat_boxlist,
    remove_small_boxes,
...
def convert_grounding_to_od_logits(logits, box_cls, positive_map, score_agg=None):
    ...
    scores = torch.zeros(logits.shape[0], logits.shape[1], box_cls.shape[2]).to(
        logits.device
    )
...
def convert_grounding_to_od_logits_v2(
    logits, num_class, positive_map, score_agg=None, disable_minus_one=True
):
        ...
        if score_agg == "MEAN":
            ...
            for label_j in positive_map:
                ...
                probability = torch.prod(
                    logits[:, :, torch.LongTensor(locations_label_j)], dim=-1
                ).squeeze(-1)
                probability = torch.pow(probability, 1 / len(locations_label_j))
                scores[
                    :, :, label_j if disable_minus_one else label_j - 1
                ] = probability
        elif score_agg == "MAX":
            # torch.max() returns (values, indices)
            for label_j in positive_map:
