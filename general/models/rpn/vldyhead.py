...
class CLIPTransformerLayer(nn.Module):
    def __init__(self):
        ...
        self.cfg = cfg
        d_model = self.cfg.MODEL.CLIP.WIDTH
        n_head = self.cfg.MODEL.CLIP.HEADS
        drop_path = self.cfg.MODEL.CLIP.DROP_PATH
...
class VLFuse(torch.nn.Module):
    ...
    def init_configs(self, cfg):
        # common params
        self.lang_model = cfg.MODEL.LANG.BODY
        ...
        self.max_query_len = cfg.MODEL.LANG.MAX_QUERY_LEN
        self.n_layers = cfg.MODEL.LANG.N_LAYERS
        self.coord_dim = 8
        ...
        if self.lang_model in ["bert-base-uncased", "roberta-base", "clip"]:
            self.lang_dim = cfg.MODEL.LANG.LANG_DIM
        else:
            self.lang_dim = 1024
...
class VLDyHead(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.cfg = cfg
        # bert_cfg = BertConfig.from_pretrained(cfg.MODEL.LANG.BODY)
        if cfg.MODEL.LANG.BODY == "bert-base-uncased":
            lang_cfg = BertConfig.from_pretrained(cfg.MODEL.LANG.BODY)
        elif cfg.MODEL.LANG.BODY == "clip":
            lang_cfg = cfg
        else:
            lang_cfg = None
        ...
        num_tokens = cfg.MODEL.LANG.MAX_QUERY_LEN
        num_anchors = len(cfg.MODEL.RPN.ASPECT_RATIOS) * cfg.MODEL.RPN.SCALES_PER_OCTAVE
        in_channels = cfg.MODEL.VISION.OUT_CHANNELS
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:
            ...
            self.bias_lang = nn.Parameter(
                torch.zeros(self.cfg.MODEL.LANG.LANG_DIM),
                requires_grad=True,
            )
            self.bias0 = nn.Parameter(torch.Tensor([bias_value]), requires_grad=True)
