...
class FFC_SE(nn.Module):
    def __init__(self, channels, ratio_g):
        super(FFCSE, self).__init__()
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        ffc3d = False
        fft_dim = (-3, -2, -1) if ffc3d else (-2, -1)
        ffted = torch.fft.rfftn(x, dim=fft_dim, norm='ortho') # (batch, c, h, w/2+1, 2)
        ...
        output = torch.fft.irfft(ffted, dim=fft_dim, signal_sizes=r_size[2:], norm='ortho')
