...
class PersonKeypoints(Keypoints):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(cfg.MODEL.ROI_KEYPOINT_HEAD.KEYPOINT_NAME) > 0:
            ...
            self.FLIP_MAP = {
                l: r
                for l, r in PersonKeypoints._FLIP_MAP.items()
                if l in cfg.MODEL.ROI_KEYPOINT_HEAD.KEYPOINT_NAME
            }
        else:
    ...
    def to_coco_format(self):
        ...
        for i in range(self.keypoints.shape[0]):
            ...
            for ki, name in enumerate(self.NAMES):
                coco_kps[3 * PersonKeypoints._NAMES.index(name)] = self.keypoints[
                    i, ki, 0
                ].item()
                coco_kps[3 * PersonKeypoints._NAMES.index(name) + 1] = self.keypoints[
                    i, ki, 1
                ].item()
                coco_kps[3 * PersonKeypoints._NAMES.index(name) + 2] = self.keypoints[
                    i, ki, 2
                ].item()
