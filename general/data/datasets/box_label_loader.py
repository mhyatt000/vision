...
class LabelLoader(object):
    ...
    def add_scores_all(self, annotations):
        ...
        for obj in annotations:
            scores_all.append(
                np.frombuffer(base64.b64decode(obj["scores_all"]), np.float32)
            )
    ...
    def add_boxes_all(self, annotations):
        ...
        for obj in annotations:
            boxes_all.append(
                np.frombuffer(base64.b64decode(obj["boxes_all"]), np.float32).reshape(
                    -1, 4
                )
            )
    ...
    def relation_loader(self, relation_annos, target):
        ...
        for i in range(len(relation_annos)):
            if (
                len(self.ignore_rel) != 0
                and relation_annos[i]["class"] in self.ignore_rel
            ):
                continue
    ...
    def __call__(self, annotations, img_size, remove_empty=True):
        ...
        for field in self.extra_fields:
            ...
            if field == "class":
                ...
                is_group = [
                    1 if "IsGroupOf" in obj and obj["IsGroupOf"] == 1 else 0
                    for obj in annotations
                ]
                target.add_field("IsGroupOf", torch.tensor(is_group))
            elif field == "IsProposal":
                is_proposal = [
                    1 if "IsProposal" in obj and obj["IsProposal"] == 1 else 0
                    for obj in annotations
                ]
