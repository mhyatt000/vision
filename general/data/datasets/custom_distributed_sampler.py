...
class DistributedSamplerChunkByNode(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        addition_indices = self.generate_indices_within_range_with_rank(
            ...
            valid_indices=list(
                range(self.current_node_start_range, self.current_node_end_range)
            ),
            prefix="Distribute ",
        )
        ...
        random.seed(
            self.seed + self.epoch + 10 * self.rank
        )  # Set the seed to maximize randomness
    ...
    def generate_indices_within_range_with_rank(
        ...
        if generate_length != -1:
            if len(indices) > generate_length:
                ...
                indices.extend(
                    np.random.choice(
                        valid_indices, generate_length - len(indices)
                    ).tolist()
                )
        return indices
