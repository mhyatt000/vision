...
class CaptionTSV(TSVYamlDataset):
    ...
    def pack_caption(
        self, positive_caption, negative_captions, original_tokens_positive
    ):
    ...
    def __get_negative_captions__(self, idx, negative_size=7):
        ...
        for i in range(negative_size):
            img, anno, _, scale = super(CaptionTSV, self).__getitem__(
                np.random.choice(len(self))
            )
            ...
            if self.inference_caption:
                ...
                try:
                    if self.further_screen:
                        ...
                        bboxes = bboxes[keep]
                        tokens_positive = [
                            i for index, i in enumerate(tokens_positive) if keep[index]
                        ]
                        ...
                        if nms_thre > 0:
                            keep = nms(
                                boxes=bboxes, scores=scores, iou_threshold=nms_thre
                            )
                            scores = scores[keep]
                            bboxes = bboxes[keep]
                    ...
                    if empty_everything:
                        caption = self.__get_negative_captions__(idx, negative_size=1)[
                            0
                        ]
                    ...
                    if self.pack_random_caption_number != 0:
                        if self.random_pack_prob != -1.0:
                            if random.random() < self.no_random_pack_probability:
                                ...
                                negative_pack_number = np.random.choice(
                                    self.pack_random_caption_number
                                )
                        else:
                            negative_pack_number = self.pack_random_caption_number

                        negative_captions = self.__get_negative_captions__(
                            idx, negative_size=negative_pack_number
                        )
