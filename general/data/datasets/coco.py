...
class COCODataset(CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if anno and "keypoints" in anno[0]:
            keypoints = []
            gt_keypoint = self.coco.cats[1][
                "keypoints"
            ]  # <TODO> a better way to get keypoint description
            ...
            for obj in anno:
                if len(use_keypoint) > 0:
                    ...
                    for name in use_keypoint:
                        kp_idx = slice(
                            3 * gt_keypoint.index(name), 3 * gt_keypoint.index(name) + 3
                        )
        ...
        if cfg.DATASETS.SAMPLE_RATIO != 0.0:
            ...
            num_sample_target = (
                math.ceil(len(target) * ratio) if ratio > 0 else math.ceil(-ratio)
            )
