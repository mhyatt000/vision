...
class VGTSVDataset(TSVYamlDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if "train" in op.basename(self.linelist_file):
            ...
            raise ValueError(
                "Split must be one of [train, test], but get {}!".format(
                    self.linelist_file
                )
            )
        ...
        assert all(
            [
                self.classes[i] == self.ind_to_class[str(i)]
        ...
        assert all(
            [
                self.attributes[i] == self.ind_to_attribute[str(i)]
        ...
        assert all(
            [
                self.relations[i] == self.ind_to_relation[str(i)]
    ...
    def _get_freq_prior(self, must_overlap=False):
        fg_matrix = np.zeros(
            (len(self.classes), len(self.classes), len(self.relations)), dtype=np.int64
        )
    ...
    def get_target_from_annotations(self, annotations, img_size, idx):
        if self.is_load_label and annotations:
            ...
            if self.split in ["val", "test"]:
                ...
                target.add_field(
                    "difficult", torch.zeros(len(target), dtype=torch.int32)
                )
            # load relations
            if self.relation_on:
