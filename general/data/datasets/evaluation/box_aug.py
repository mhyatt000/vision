    ...
    for i in range(num_images):
        ...
        if number_of_detections > cfg.TEST.PRE_NMS_TOP_N > 0:
            ...
            image_thresh, _ = torch.kthvalue(
                cls_scores.cpu(), number_of_detections - cfg.TEST.PRE_NMS_TOP_N + 1
            )
...
def boxlist_nms(
    boxlist, thresh, max_proposals=-1, score_field="scores", nms_type="nms"
):
    if thresh <= 0:
        return boxlist
    mode = boxlist.mode
...
def soft_bbox_vote(boxes, scores, vote_thresh):
    ...
    while det.shape[0] > 0:
        ...
        if merge_index.shape[0] <= 1:
            ...
            soft_index = np.where(
                soft_det_accu[:, 4] >= cfg.MODEL.RETINANET.INFERENCE_TH
            )[0]
