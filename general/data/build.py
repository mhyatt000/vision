...
def build_dataset(
    ...
    for dataset_id, dataset_name in enumerate(dataset_list, 1):
        ...
        if is_train:
            dataset_name += cfg.DATASETS.TRAIN_DATASETNAME_SUFFIX
        else:
            dataset_name += cfg.DATASETS.TEST_DATASETNAME_SUFFIX
...
def make_data_sampler(
    dataset, shuffle, distributed, num_replicas=None, rank=None, use_random_seed=True
):
...
def make_data_loader(is_train=True, num_replicas=None, rank=None, start_iter=0):
    ...
    if is_train:
        ...
        "of GPUs ({}) used.".format(images_per_batch, num_gpus)
        images_per_gpu = images_per_batch // num_gpus
        shuffle = False if not cfg.distributed else True
        num_iters = None
        start_iter = 0
    ...
    paths_catalog = import_file("general.config.paths_catalog", cfg.PATHS_CATALOG, True)
    ...
    if (
        ...
        for bing_index in cfg.DATASETS.BING_INDEX_LIST:
            dataset_list.insert(
                len(dataset_list), "bing_caption_{}_train_no_coco".format(bing_index)
            )
        dataset_list = tuple(dataset_list)
    ...
    if is_train and cfg.DATASETS.SAMPLE_NEGATIVE_FOR_GROUNDING_DATA > 0:
        extra_args[
            "sample_negative_for_grounding_data"
        ] = cfg.DATASETS.SAMPLE_NEGATIVE_FOR_GROUNDING_DATA
    ...
    if is_train:
        extra_args[
            "mlm_obj_for_only_positive"
        ] = cfg.MODEL.DYHEAD.FUSE_CONFIG.MLM_OBJ_FOR_ONLY_POSITIVE
    ...
    if is_train:
        ...
        ] = cfg.DATASETS.SPECIAL_SAFEGUARD_FOR_COCO_GROUNDING
    if is_train:
    ...
    if cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "clip":
        ...
        extra_args["tokenizer"] = AutoTokenizer.from_pretrained(
            cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE
        )
    ...
    if is_train and cfg.SOLVER.MULTI_MAX_EPOCH:
        cfg.defrost()
        cfg.SOLVER.MULTI_MAX_ITER += (
            cfg.SOLVER.MULTI_MAX_EPOCH[-1]
            * min([len(dataset) // cfg.SOLVER.IMS_PER_BATCH for dataset in datasets]),
        )
        cfg.freeze()
