...
from general.data.datasets.evaluation import evaluate, im_detect_bbox_aug
from general.data.datasets.evaluation.flickr.flickr_eval import FlickrEvaluator
from general.data.datasets.tsv import load_from_yaml_file
...
def create_queries_and_maps_from_dataset(dataset, cfg):
    ...
    for i in range(len(labels)):
        ...
        query_i, positive_map_label_to_token_i = create_queries_and_maps(
            labels_i,
            label_list_i,
            additional_labels=cfg.DATASETS.SUPRESS_QUERY
            if cfg.DATASETS.USE_SUPRESS_QUERY
            else None,
            cfg=cfg,
        )
...
def resize_box(output, targets):
    ...
    if isinstance(targets[0], dict):
        ...
        orig_target_sizes = torch.stack(
            [targets[0].extra_fields["orig_size"] for _ in range(1)], dim=0
        )
...
def flickr_post_process(output, targets, positive_map_label_to_token, plus):
    ...
    scores, indices = torch.topk(
        output.extra_fields["scores"], k=len(output.extra_fields["scores"]), sorted=True
    )
...
def build_lvis_evaluator(ann_file, fixed_ap=True):
    ...
    from general.data.datasets.evaluation.lvis.lvis import LVIS
    from general.data.datasets.evaluation.lvis.lvis_eval import (
        LvisEvaluator,
        LvisEvaluatorFixedAP,
    )
...
def inference( model, data_loader, dataset_name, iou_types=("bbox",),
    ...
    if "flickr" in cfg.DATASETS.TEST[0]:
        ...
        evaluator = build_lvis_evaluator(
            dataset.ann_file, fixed_ap=not cfg.DATASETS.LVIS_USE_NORMAL_AP
        )
    else:
        evaluator = None
    ...
    for i, batch in enumerate(_iterator):
        ...
        with torch.no_grad():
            if cfg.TEST.USE_MULTISCALE:
                ...
                output = im_detect_bbox_aug(
                    model, images, device, captions, positive_map_label_to_token
                )
                output = [o.to(cpu_device) for o in output]
                ...
                for query_i in range(query_time):
                    if not isinstance(
                        targets[0], dict
                    ):  # For LVIS dataset and datasets directly copied from MDETR
        ...
        if visualizer is not None:
            ...
            try:
                image_path = os.path.join(
                    dataset.root, dataset.coco.loadImgs(image_id)[0]["file_name"]
                )
    ...
    if evaluator is not None:
        ...
        print(score)
        import general.utils.mdetr_dist as dist

        if is_main_process():
            if "flickr" in cfg.DATASETS.TEST[0]:
                write_flickr_results(
