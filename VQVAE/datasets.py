...
class MiniImagenet(data.Dataset):
    ...
    def __init__( self, root, train=False, valid=False, test=False, transform=None, target_transform=None, download=False):
        ...
        if not (((train ^ valid ^ test) ^ (train & valid & test))):
            raise ValueError(
                ...
                "must be True (train={0}, valid={1}, test={2}).".format(train, valid, test)
            )
