...
from modules import GatedPixelCNN, VectorQuantizedVAE
...
def main(args):
    ...
    prior = GatedPixelCNN(
        args.k, args.hidden_size_prior, args.num_layers, n_classes=len(train_dataset._label_encoder)
    ).to(args.device)
...
if __name__ == "__main__":
    ...
    # Latent space
    parser.add_argument(
        "--hidden-size-vae", type=int, default=256, help="size of the latent vectors (default: 256)"
    )
    parser.add_argument(
        "--hidden-size-prior",
        type=int,
    ...
    parser.add_argument(
        "--lr", type=float, default=3e-4, help="learning rate for Adam optimizer (default: 3e-4)"
    )
    ...
    parser.add_argument(
        "--device", type=str, default="cpu", help="set the device (cpu or cuda, default: cpu)"
    )
